...
fig.cap="Model 4 Test: Independence among the residuals"

```{r EliottTable, echo=FALSE, fig.cap="Displaying Differences in Trust and Faith Change"}
...
```{r SimoneChunkCleaning, include=FALSE}
...
sm_pewlog<- sm_pew %>%
 select(covid_united, covid_family, country) %>%
  zap_labels()
      ...
      mutate(covid_family = dplyr::recode(factor(covid_family,
                '2' = 1, 
                '3' = 2,
                '1' = 3)),
               covid_united = dplyr::recode(factor(covid_united,
                                      '1' = 2,
                                      '2' = 1)))
sm_pewlog<- sm_pew %>%
 select(covid_united, covid_family, country) %>%
...
```{r SimoneRegressions, echo = FALSE, fig.cap="Regression Plot: Predicting Perceptions of National Unity by Change in Family Closeness during COVID"}
